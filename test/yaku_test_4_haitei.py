...
async def test_haitei_junchan_sanshoku(base_url: str, headless: bool = True):
    ...
    async with async_playwright() as p:
        ...
        try:
            ...
            await page.get_by_role("textbox", name="ツモ牌 (順番通り) ツモ牌 (順番通り)").fill("sha sha sha sha sha sha sha sha sha sha sha sha sha sha sha sha sha 9p")
            await page.get_by_role("tab", name="プレイヤー1(自分)").click()
            await page.get_by_role("tab", name="プレイヤー2").click()
            await page.wait_for_timeout(2000)

            await page.get_by_role("textbox", name="手牌 (13枚または14枚) 手牌 (13枚または14枚)").click()
            await page.get_by_role("textbox", name="手牌 (13枚または14枚) 手牌 (13枚または14枚)").fill("1m 2m 3m 1p 2p 3p 1s 2s 3s ton ton nan nan ")
            await page.get_by_role("textbox", name="ツモ牌 (順番通り) ツモ牌 (順番通り)").click()
            await page.get_by_role("textbox", name="ツモ牌 (順番通り) ツモ牌 (順番通り)").fill("sha sha sha sha sha sha sha sha sha sha sha sha sha sha sha sha sha 9p sha sha sha sha sha sha sha sha sha sha sha sha sha sha")
